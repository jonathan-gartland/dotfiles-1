...
gui_editor = "subl"
graphics   = "gimp"
mail       = "thundebird" -- terminal .. " -e mutt "

-- Need to auto-start the following:
-- pulse-audio systray
run_once("pasystray")
-- Bittorrent sync
run_once("btsync --config /home/skk/.config/btsync/btsync.conf")
...
local tile_left = layouts[2]
local max = layouts[8]

-- {{{ Tags
tags = {
   names =  { "web",     "subl",    "term",    "email",  "media",    "files",   "other", "games" },
   layout = { tile_left, tile_left, tile_left, tile_left, tile_left, tile_left, tile_left, max  }
}
...
yawn = lain.widgets.yawn(2393367, {
    u = "f",
    settings = function()
...
mailwidget = lain.widgets.maildir({
    timeout  = 30,
    mailpath ="/home/skk/Maildir/knightcx",
    ignore_boxes = {},
    settings = function()
        if mailcount > 0 then
